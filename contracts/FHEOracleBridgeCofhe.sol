...
import "./libraries/MedianLibCofhe.sol";
import "@fhenixprotocol/cofhe-contracts/FHE.sol";

contract FHEOracleBridgeCofhe is IFHEOracleBridgeCofhe {
    ...
    /// @notice Emitted after a submission when `minFeeders` is not yet met — aggregate/median is not updated until quorum.
    event QuorumPending(
        ...
        feedExists(feedId)
    {
        ...
        require(!submissions[feedId][currentRound][msg.sender].submitted, "Oracle: already submitted this round");

        euint128 enc = FHE.asEuint128(encPrice);

        submissions[feedId][currentRound][msg.sender] = FeederSubmission({ price: enc, submitted: true });
        roundFeeders[feedId][currentRound].push(msg.sender);

        emit PriceSubmitted(feedId, msg.sender, currentRound);

        uint256 n = roundFeeders[feedId][currentRound].length;
        if (n >= uint256(feed.minFeeders)) {
            _finaliseRound(feedId, currentRound);
        } else {
            emit QuorumPending(feedId, currentRound, uint8(n), feed.minFeeders);
        }
    }
    ...
    function _finaliseRound(uint256 feedId, uint256 roundId_) internal {
        ...
        if (n == 1) {
            ...
            aggregated = MedianLibCofhe.encryptedMedian(prices);
        }
        ...
        // allow contract + all whitelisted consumers to access handle
        FHE.allowThis(aggregated);
        address[] memory consumers = registry.allConsumers();
        for (uint256 i = 0; i < consumers.length; i++) {
            if (registry.isWhitelisted(consumers[i])) {
                FHE.allow(aggregated, consumers[i]);
            }
        }
